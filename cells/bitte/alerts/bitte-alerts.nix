    ...
    rules = [
      {
        ...
        annotations = {
          dashboard = ''{{ $externalURL }}/explore?left={"datasource":"Loki","queries":[{"datasource":"Loki","expr":"sum(rate({syslog_identifier=\"systemd-coredump\"}[1h] != \"sshd\" |= \"dumped core\")) by (host) > 0","refId":"A"}],"range":{"from":"now-1h","to":"now"}}&orgId=1'';
          description = "Detected a coredump on {{ $labels.host }}";
          summary = "Detected a coredump on {{ $labels.host }}. This usually requires attention and most likely manual intervention. To analyze a coredump, run `coredumpctl list` on the affected machine, and run `coredump debug $id` in a nix shell with gdb.";
        };
